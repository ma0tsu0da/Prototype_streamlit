...
fukuoka_df = fukuoka_df[(fukuoka_df["市区町村"] != "福岡市") & (fukuoka_df["市区町村"] != "北九州市")]

map_center = [33.590204, 130.420850]  # 福岡
my_map = folium.Map(location=map_center, tiles='cartodbpositron', zoom_start=9)

population = folium.Choropleth(                   # コロプレス図を表示する
    geo_data=fukuoka,                             # 市区町村の形が入ったデータ
    data=fukuoka_df,                              # 色分けの基準が入ったデータ
    columns=["市区町村コード", "2023年の人口"],   # dataで指定したdfの、geo_dataと対応するカラムと色分け基準カラム
...
income = folium.Choropleth(
    ...
    bins=list(
        fukuoka_df["一人当たり課税対象所得(百万円)"].quantile([
...
population_density = folium.Choropleth(
    ...
    bins=list(
        fukuoka_df["2023年の人口密度"].quantile([
            0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1])),
    show=False,
)
...
for cp in choropleth_list:
    ...
    #     s["properties"]["population"] = data_indexed.loc[s["id"], "2023年の人口"]
    folium.GeoJsonTooltip(["N03_004"], ["地域"]).add_to(cp.geojson)

stations_cluster = MarkerCluster(
    maxClusterRadius=40, disableClusteringAtZoom=13, name="駅").add_to(my_map)
